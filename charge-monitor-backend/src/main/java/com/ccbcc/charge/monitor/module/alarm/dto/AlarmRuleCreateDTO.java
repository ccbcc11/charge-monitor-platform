 ...
 */
@Data
@Schema(description = "新增告警规则请求参数")
public class AlarmRuleCreateDTO implements Serializable {
    ...
    @Pattern(regexp = "^[A-Z][A-Z0-9_]*$", message = "规则编码必须以大写字母开头，只能包含大写字母、数字和下划线")
    @Schema(description = "规则编码，唯一，建议以 RULE_ 开头", example = "RULE_TEMP_HIGH", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description = "规则名称", example = "设备温度过高", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description = "阈值", example = "85.00", requiredMode = Schema.RequiredMode.REQUIRED)
