...
| 文档版本 | v1.0 / MVP 第一版 |
| 适用阶段 | 第一阶段：MVP 开发 |
| 编写目的 | 明确告警类型、触发规则、告警等级、去重策略、状态流转和后续扩展方向 |
...
```text
设备运行数据上报 → 基础阈值判断 → 生成告警记录 → 告警查询 → 告警确认 → 告警恢复
```

第二阶段再升级为：

```text
设备运行数据上报 → RabbitMQ 异步告警 → WebSocket 实时推送 → 自动创建工单 → 工单处理闭环
```
...
5. **便于扩展**：MVP 写死规则，第二阶段迁移到 `alarm_rule` 表动态配置。
...
| 离线告警 | OFFLINE | 设备超过指定时间未上报心跳 | 预留，第二阶段完善 |
| 连续异常告警 | CONTINUOUS | 连续 N 次指标异常 | 预留，第二阶段完善 |
...
MVP 第一版先在代码中写死以下规则，后续再改为数据库配置。

| 编号 | 指标 | 触发条件 | 告警等级 | 告警类型 | 告警说明 |
|---|---|---|---|---|---|
| R001 | temperature | temperature > 80 | 3 严重 | THRESHOLD | 设备温度过高 |
| R002 | voltage | voltage < 180 | 2 重要 | THRESHOLD | 设备电压过低 |
| R003 | network_delay | networkDelay > 200 | 1 一般 | THRESHOLD | 网络延迟过高 |
...
## 13. 第二阶段 alarm_rule 表设计

MVP 第一版规则写死在代码中，第二阶段建议使用 `alarm_rule` 表动态配置。
...
|---|---|---|---|---:|---:|
| R001 | 温度过高告警 | temperature | > | 80 | 3 |
| R002 | 电压过低告警 | voltage | < | 180 | 2 |
| R003 | 网络延迟过高告警 | network_delay | > | 200 | 1 |
...
MVP 第一版告警模块采用“简单、可解释、能跑通”的设计：

```text
固定阈值规则 + 告警分级 + 告警去重 + 状态流转 + Redis 异常设备集合
```

它能支撑项目第一阶段核心演示：

```text
模拟设备上报异常数据 → 系统自动生成告警 → 运维人员确认 → 运维人员恢复
```

后续只需将规则判断从代码写死迁移到 `alarm_rule` 表，并接入 RabbitMQ、WebSocket 和工单模块，就可以自然升级为完整的智能运维告警闭环平台。
