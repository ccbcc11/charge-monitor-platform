...
| 表名 | 中文名 | 作用 |
|---|---|---|
| sys_user | 用户表 | 登录认证、用户身份 |
| sys_role | 角色表 | 管理员、运维人员、只读用户 |
| sys_user_role | 用户角色关联表 | 用户与角色多对多关系 |
| device_info | 设备信息表 | 充电桩/终端设备台账 |
| device_data | 设备运行数据表 | 设备历史遥测数据 |
| alarm_rule | 告警规则表 | 动态配置告警规则 |
| alarm_record | 告警记录表 | 异常告警记录、状态和次数 |
| work_order | 工单表 | 告警自动建单和工单流转 |
...
## 8. alarm_rule 告警规则表（已实现）

| 字段名 | 类型 | 说明 |
|---|---|---|
| id | bigint | PK |
| rule_code | varchar(64) | 规则编码，UNIQUE |
| rule_name | varchar(100) | 规则名称 |
| alarm_type | varchar(50) | THRESHOLD / OFFLINE / CONTINUOUS / FLUCTUATION |
| device_type | varchar(50) | 适用设备类型，NULL=全部 |
| metric_name | varchar(50) | 指标名称 |
| operator | varchar(10) | >、>=、<、<=、=、!= |
| threshold_value | decimal(10,2) | 阈值 |
| window_size | int | 滑动窗口大小 |
| trigger_count | int | 触发次数 |
| alarm_level | tinyint | 1一般 2重要 3严重 |
| enabled | tinyint | 0禁用 1启用 |

管理接口：`POST/PUT/DELETE/GET /api/alarm/rule` + enable/disable。

## 9. work_order 运维工单表（已实现）

| 字段名 | 类型 | 说明 |
|---|---|---|
| id | bigint | PK |
| work_order_no | varchar(64) | 工单编号，UNIQUE |
| alarm_id | bigint | 关联告警ID，UNIQUE（一对一） |
| device_id | bigint | 设备ID |
| device_code | varchar(64) | 设备编号 |
| title | varchar(200) | 工单标题 |
| content | varchar(1000) | 工单内容 |
| priority | tinyint | 1低 2中 3高 |
| status | tinyint | 0待处理 1处理中 2已完成 3已关闭 |
| assignee_id | bigint | 处理人ID |
| assignee_name | varchar(100) | 处理人名称 |
| finish_time | datetime | 完成时间 |

管理接口：`GET /api/work-order/page` + detail + accept/finish/close/delete。
...
```text
用户登录 → 设备管理 → 数据上报 → 动态规则检测
→ 告警生成 → 实时推送 → 自动工单 → 工单处理 → 报表概览
```
